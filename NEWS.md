...
* `pypsa_eur_41v` reworked: the existing fleet is one `STOCK` vintage per
  carrier carrying the summed retirement path (the build-year bins were
  parameter-flat and are aggregated away); gas gains `NEW<year>` vintage
  windows with each assumption year's efficiency, lifetime and annuity
  frozen, replacing an open path whose year-keyed efficiency read as a free
  retrofit of standing capacity; cost-only carriers keep a single year-keyed
  investment path. The fleet's milestone totals are unchanged.
...
* The translation article gains a "Revised assumptions" section documenting
  the deliberate deviations shipped as separate model versions: economic
  retirement and TYNDP demand growth.
