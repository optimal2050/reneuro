...
# built model and are written out rather than derived.
library(reneuro)
...
Eleven models ship with the package, ready to solve — all built on **2025
weather and load** (the `europe-2025-sarah3-era5` cutout; measured ENTSO-E
demand). They need neither Python nor a PyPSA-Eur clone, and are lazy-loaded,
so they cost nothing until touched.

Four span spatial resolutions. Each models a **single planning year** — 2050
costs, the 2025 fleet as existing stock — resolved hour by hour within it:
...
`cluster_network` workflow and read straight into energyRt, so each is directly
comparable to a PyPSA-Eur run at the same size.

`pypsa_eur_nuts3` follows **NUTS** — *Nomenclature des Unités Territoriales
Statistiques*, Eurostat's hierarchy of European regions — rather than an
algorithmic clustering, which is what lets the model be joined to published
statistics. It is a **source** model rather than one to solve whole: its
purpose is to supply a country or study area at full granularity, or a coarser
continental model by aggregation. It covers the full year; its hourly weather
is most of its size, so it ships as separate objects that `attach_weather()`
puts back.

### Planning across years

Seven more models take the 41-node system through 2050. Their calendar is the
same full hourly year; what varies is the **planning horizon**:
...
horizon"; `pypsa_eur_41v` optimizes the whole pathway at once, with per-vintage
efficiency and retirement. All seven ship **without weather** —
`attach_weather(m, from = pypsa_eur_41)` puts `pypsa_eur_41`'s profiles back,
the same pattern `pypsa_eur_nuts3` uses.
...
alongside `reneuro`. A solver is also required, and **HiGHS through Julia or
Python is the one to install** — GLPK solves the five-node models below, but
...
- [Data sources](https://optimal2050.github.io/reneuro/articles/data-sources.html)
  — the upstream inputs as maps and figures, with the source and licence table
