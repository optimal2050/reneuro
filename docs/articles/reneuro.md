...
set_default_solver(solver_options$glpk)     # GLPK for small models
```
...
puts back the ones the study needs. Use `pypsa_eur_1035` instead for the
same regions on a four-day sample, which carries its weather already.
