...
- `pypsa_eur_41v` reworked: the existing fleet is one `STOCK` vintage
  per carrier carrying the summed retirement path, and gas gains
  `NEW<year>` vintage windows that freeze each assumption year’s
  efficiency, lifetime and annuity. The fleet’s milestone totals are
  unchanged.
...
- In `policy_paths` the ETS-shape scenario is named `ets_cap` (was
  ...
  tightest of the three paths, and the old name misread as a
  contradiction.
...
- The translation article gains a “Revised assumptions” section
  ...
  versions: atlas-anchored capacity factors, economic retirement, and
  TYNDP demand growth.
...
- The website’s data documentation is reorganized into four articles:
  “Data in PyPSA-Eur 41-node” (what ships in the 41-node model, with
  mean capacity-factor, demand and network maps), “Data sources” (the
  upstream inputs and the NUTS processing), and new “Wind energy” and
