...
`pypsa_eur_nuts3` is the same NUTS3 network over the **full year**
(8,760 hourly snapshots). Its hourly weather profiles are seven eighths
...
so `energyRt::aggregate_model_regions(pypsa_eur_nuts3, level = "nuts1")`
needs no second argument; the coarser NUTS levels are derived that way
rather than shipped.
